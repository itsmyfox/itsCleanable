...
echo ===
echo Windows.edb является индексной базой данных службы поиска Windows. Благодаря такой индексации поиск происходит быстрее и эффективнее. Необходимо ли удалять Windows.edb и генерировать новый фаил для освобождения пространства? (Y/N)
choice /m "Windows.edb является индексной базой данных службы поиска Windows. Благодаря такой индексации поиск происходит быстрее и эффективнее. Необходимо ли удалять Windows.edb и генерировать новый фаил для освобождения пространства?"
